...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const canonicalUrl = `${siteUrl}/keywords/${item.slug}`;
...
export default async function KeywordDetailPage({ params }: PageProps) {
  ...
  const jsonLdArticle = {
    ...
    "mainEntityOfPage": {
      "@type": "WebPage",
      "@id": `https://simonwerdetail.info/keywords/${item.slug}`
    }
  };
  ...
  const jsonLdBreadcrumb = {
    ...
    "itemListElement": [
      ...
      },
      {
        ...
        "position": 4,
        "name": item.title,
        "item": `https://simonwerdetail.info/keywords/${item.slug}`
      }
    ]
  };
